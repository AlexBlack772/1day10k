...
contract ICurveFiCurve {
   ...
   //withdraw_admin_feesとは、管理者手数料を引き出す
   ...
   //admin_feeとは、管理者手数料を取得する
   function admin_fee() external view returns (uint256);
   //ownerとは、所有者を取得する
   function owner() external view returns (address);
   //admin_actions_deadlineとは、管理者アクションの期限を取得する
   function admin_actions_deadline() external view returns (uint256);
   //tranfer_ownership_deadlineとは、所有権の移転期限を取得する
   function transfer_ownership_deadline() external view returns (uint256);
   //future_feeとは、未来の手数料を取得する
   function future_fee() external view returns (uint256);
   //future_admin_feeとは、未来の管理者手数料を取得する
   function future_admin_fee() external view returns (uint256);
   //future_ownerとは、未来の所有者を取得する
