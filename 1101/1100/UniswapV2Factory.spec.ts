...
describe('UniswapV2Factory', () => {
  ...
  //createFixtureLoaderは、テストの前に実行される関数
  const loadFixture = createFixtureLoader(provider, [wallet, other]);

  let factory: Contract;
  beforeEach(async () => {
    //loadFixtureとは、テストの前に実行される関数
    const fixture = await loadFixture(factoryFixture);
    factory = fixture.factory;
  });

  it("feeTo, feeToSetter, allPairsLength", async () => {
    //feeToの初期値は、AddressZero
    expect(await factory.feeTo()).to.eq(AddressZero);
    //feeToSetterの初期値は、wallet.address
    expect(await factory.feeToSetter()).to.eq(wallet.address);
    //allPairsLengthの初期値は、0
    expect(await factory.allPairsLength()).to.eq(0);
  });

  //createPairとは、factoryの中にある関数
  async function createPair(tokens: [string, string]) {
    //evm.bytecode.objectは、コンパイルしたバイトコード
    const bytecode = `0x${UniswapV2Pair.evm.bytecode.object}`;
    //getCreate2Addressは、create2のアドレスを取得する関数
