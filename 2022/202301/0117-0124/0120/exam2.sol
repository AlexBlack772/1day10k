...
//
メッセージにオフチェーンで署名し、関数を実行する前にその署名を必要とするコントラクトを持つことは、便利な手法です。

たとえば、この手法は次の目的で使用されます。

チェーン上のトランザクション数を減らす
ガスレス取引と呼ばれるmeta transaction
脆弱性
同じシグネチャを複数回使用して関数を実行できます。署名者の意図が一度トランザクションを承認することであった場合、これは有害である可能性があります。
