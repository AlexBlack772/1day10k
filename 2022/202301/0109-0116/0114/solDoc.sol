...
library ArrayUtils {
    ...
    function reduce(uint[] memory self, function (uint, uint) pure returns (uint) f, uint _initialValue)
        public
        pure
        returns (uint r)
    {
    ...
    function range(uint _length) public pure returns (uint[] memory r) {
        r = new uint[](_length);
